...
根據實體規格(Entity Spec)產生完整、可編譯的 Java Spring Boot 四層程式碼(Domain → Usecase → Adapter → Application)。所有類別名稱、package 路徑、註解位置皆由固定模板推導,確保多次產出結果高度一致。

## 觸發時機

**使用**:在 Java 專案中實作新實體或新功能、依規格 scaffold 四層架構、TDD 實作(規格附測試描述)。
...
1. **依賴方向由外向內**:Application/Adapter → Usecase → Domain。每一層只 import 自己這層與更內層的類別。
...
4. **Adapter 隔離技術細節**:JPA 註解只出現在 `<Entity>Entity.java`;`<Entity>Mapper` 全部為 static 方法;`<Entity>RepositoryImpl` 實作 Usecase 的 Outbound Port;外部 SDK 例外在此層捕捉並轉為自訂例外(如 `PaymentGatewayException`)後上拋。
5. **Application 只做轉換**:Controller 僅路由 → 呼叫 UseCase → 包裝 Response DTO;Request DTO 用 `jakarta.validation` + `@Valid`;`GlobalExceptionHandler`(`@RestControllerAdvice`)統一轉換:NotFound → 404、`IllegalArgumentException` 與驗證失敗 → 400、其他 → 500;Response DTO 包裝 Domain 物件後回傳。
...
8. **產生順序固定**:(測試 →)Domain → Usecase → Adapter → Application;有測試描述時,測試先於實作產生。
...
3. **依序產生四層**:每層產生前先讀取對應 reference 的模板:
   ...
   | Adapter | `references/adapter_layer.md` | JPA Entity、Mapper、JpaRepository、RepositoryImpl、(SDK ServiceImpl) |
   | Application | `references/application_layer.md` | Request/Response DTO、Controller、GlobalExceptionHandler |
...
- [ ] UseCaseImpl 的寫入方法都有 `@Transactional`,且不 import adapter/application 類別
- [ ] 事件在 `save()` 之後 publish
- [ ] JPA 註解只在 `<Entity>Entity.java`;Mapper 全 static
- [ ] Controller 無 if/else 業務邏輯;回傳皆為 Response DTO
- [ ] 所有類別名稱符合命名推導表
