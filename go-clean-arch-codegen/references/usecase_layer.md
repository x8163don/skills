...
這層本身不產生獨立的測試檔案。Usecase 只做 orchestration(呼叫 Outbound Port、串業務規則、決定何時 publish 事件),不藏業務邏輯,所以邏輯正確性已經由 Domain 的 aggregate test 涵蓋;它有沒有正確呼叫到 Repository/Client 等 Outbound Port,則交給 Adapter/Inbound 層的 controller-level integration test(用 testcontainer 起真實 DB,見 `references/testing_principles.md`)驗證。用 `testify/mock` 手寫假的 Outbound Port 只能驗證「有沒有呼叫 mock」,測不出跟真實 DB/SDK 的行為是否一致,所以不再產生這類測試。
